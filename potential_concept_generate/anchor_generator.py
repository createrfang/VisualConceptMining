...
class AnchorGenerator(nn.Module):
    ...
    def forward(self, image_list, feature_maps):
        grid_sizes = [feature_map.shape[0:2] for feature_map in feature_maps]
        anchors_over_all_feature_maps = self.grid_anchors(grid_sizes)
        anchors = []
        for i, (image_height, image_width) in enumerate([im.shape[1:3] for im in image_list]):
            anchors_in_image = []
            for anchors_per_feature_map in anchors_over_all_feature_maps:
                ...
                boxlist = boxlist.clip_to_image(True)
